...
export const createCats = (Cats) => {
  const mutation = gql`
    mutation CreateCats($input: [CatInput]!) {
      createCats(Cats: $input) {
        Name
        ID
        InternalID
        LitterGroupId
  ...
  `;
  return graphQLClient.request(mutation, { input: Cats });
};

export const updateCats = (Cats) => {
  const query = gql`
    mutation UpdateCats($Cats: [CatUpdateInput]) {
      updateCatsByInternalIds(Cats: $Cats) {
        Name
        ID
        InternalID
        LitterGroupId
...
export const getPublishableCats = () => {
  const query = gql`
    query FindCatsByStatus {
      findCatsByStatus(Status: "Adoptions: Available (On Website)") {
        data {
          Name
          ID
          Sex
...
>>>>>>> parent of f3a891a (static rendering from faunadb)
